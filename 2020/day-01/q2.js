...
const main = () => {
  for (let i = 0; i < input.length - 3; i++) {
    ...
    for (let j = i + 1; j < input.length - i - 2; j++) {
      if (input[i] + (input[j] + input[j+1]) >= TARGET) continue;

      for (let k = j + 1; k < input.length - j - i; k++) {
